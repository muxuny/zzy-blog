...
test('reading space keeps the canvas neon wave light with slow drifting particles', () => {
  const source = read('../views/ReadingSpace.vue')
  ...
  assert.match(source, /const WAVE_FREQUENCY = 0\.02/)
  assert.match(source, /const WAVE_AMPLITUDE = 24/)
  assert.match(source, /const CURSOR_RADIUS = 150/)
  assert.match(source, /const CURSOR_LIFT = 30/)
  ...
  assert.match(source, /gradient\.addColorStop\(0,\s*'#00d4ff'\)/)
  assert.match(source, /gradient\.addColorStop\(1,\s*'#7b2ffc'\)/)
  ...
  assert.match(source, /\.signal-wave\s*\{[^}]*bottom:\s*18px;[^}]*height:\s*88px;[^}]*background:\s*transparent;[^}]*pointer-events:\s*auto;/)
