...
test('reading space turns the continuation blank area into an integrated kinetic toy', () => {
  const source = read('../views/ReadingSpace.vue')

  assert.match(source, /class="kinetic-toy"/)
  assert.match(source, /class="toy-thread/)
  assert.match(source, /class="toy-chip/)
  assert.match(source, /class="toy-orbit/)
  assert.match(source, /@pointermove\.stop="moveKineticToy"/)
  assert.match(source, /@pointerleave\.stop="resetKineticToy"/)
  assert.match(source, /@pointerdown\.stop\.prevent="grabKineticToy"/)
  assert.match(source, /@click\.stop\.prevent="shuffleKineticToy"/)
  assert.match(source, /--toy-a-x/)
  assert.match(source, /--toy-tilt/)
  assert.match(source, /\.kinetic-toy\s*\{[^}]*bottom:\s*22px;[^}]*height:\s*112px;[^}]*pointer-events:\s*auto;/)
  assert.match(source, /\.kinetic-toy\s*\{(?:(?!\}).)*background:\s*transparent;/s)
  assert.doesNotMatch(source, /\.kinetic-toy\s*\{(?:(?!\}).)*border:/s)
  assert.doesNotMatch(source, /\.kinetic-toy\s*\{(?:(?!\}).)*box-shadow:/s)
  assert.match(source, /\.toy-chip\s*\{[^}]*animation:\s*toyIdle/)
  assert.match(source, /\.continue-panel\.is-toy-shuffling\s+\.toy-chip\.is-a\s*\{[^}]*animation:\s*toyShuffleA/)
  assert.match(source, /\.continue-panel\.is-toy-grabbing\s+\.kinetic-toy\s*\{[^}]*cursor:\s*grabbing;/)
  assert.match(source, /@keyframes toyIdle/)
  assert.match(source, /@keyframes toyShuffleA/)
  assert.match(source, /@keyframes toyShuffleB/)
  assert.match(source, /@keyframes toyShuffleC/)
  assert.match(source, /@media \(prefers-reduced-motion: reduce\)[\s\S]*\.kinetic-toy::before,\s*\.kinetic-toy::after,\s*\.toy-chip,\s*\.toy-thread,\s*\.toy-orbit[\s\S]*?\{[^}]*animation:\s*none;/)
  ...
  assert.doesNotMatch(source, /overview\.historyTotal[\s\S]{0,700}kinetic-toy/)
  assert.doesNotMatch(source, /overview\.favoriteTotal[\s\S]{0,700}kinetic-toy/)
})
