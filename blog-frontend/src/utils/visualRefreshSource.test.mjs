...
test('reading space turns the continuation blank area into a canvas neon wave with particles', () => {
  const source = read('../views/ReadingSpace.vue')
  ...
  assert.match(source, /const WAVE_AMPLITUDE = 60/)
  assert.match(source, /const CURSOR_RADIUS = 150/)
  assert.match(source, /const CURSOR_LIFT = 80/)
  assert.match(source, /const PARTICLE_MAX = 40/)
  assert.match(source, /const PARTICLE_LIFETIME = 1500/)
  assert.match(source, /const PARTICLE_MIN_PER_FRAME = 5/)
  assert.match(source, /const PARTICLE_MAX_PER_FRAME = 8/)
  ...
  assert.match(source, /ctx\.lineWidth = 2/)
  ...
  assert.match(source, /\.signal-wave\s*\{[^}]*bottom:\s*18px;[^}]*height:\s*128px;[^}]*background:\s*transparent;[^}]*pointer-events:\s*auto;/)
