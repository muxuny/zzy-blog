...
test('reading space turns the continuation blank area into an integrated neon signal wave', () => {
  const source = read('../views/ReadingSpace.vue')

  assert.match(source, /class="signal-wave"/)
  assert.match(source, /class="wave-svg"/)
  assert.match(source, /class="wave-path is-glow"/)
  assert.match(source, /class="wave-path is-core"/)
  assert.match(source, /class="wave-spark"/)
  assert.match(source, /@pointermove\.stop="moveSignalWave"/)
  assert.match(source, /@pointerleave\.stop="resetSignalWave"/)
  assert.match(source, /@pointerdown\.stop\.prevent="pulseSignalWave"/)
  assert.match(source, /--wave-cursor-x/)
  assert.match(source, /--wave-lift/)
  assert.match(source, /\.signal-wave\s*\{[^}]*bottom:\s*18px;[^}]*height:\s*112px;[^}]*background:\s*transparent;[^}]*pointer-events:\s*auto;/)
  assert.doesNotMatch(source, /\.signal-wave\s*\{(?:(?!\}).)*border:/s)
  assert.doesNotMatch(source, /\.signal-wave\s*\{(?:(?!\}).)*box-shadow:/s)
  assert.match(source, /\.wave-path\.is-glow\s*\{[^}]*filter:\s*drop-shadow/)
  assert.match(source, /\.continue-panel:not\(\.is-unavailable\):hover\s+\.wave-path\.is-core\s*\{[^}]*animation:\s*waveScan/)
  assert.match(source, /\.continue-panel\.is-wave-pulsing\s+\.wave-svg\s*\{[^}]*animation:\s*wavePulse/)
  assert.match(source, /@keyframes waveScan/)
  assert.match(source, /@keyframes wavePulse/)
  assert.match(source, /@media \(prefers-reduced-motion: reduce\)[\s\S]*\.wave-path,\s*\.wave-spark,\s*\.wave-svg[\s\S]*?\{[^}]*animation:\s*none;/)
