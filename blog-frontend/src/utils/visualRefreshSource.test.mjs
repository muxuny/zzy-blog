...
test('reading space turns the continuation blank area into a refined responsive neon surface line', () => {
  const source = read('../views/ReadingSpace.vue')

  assert.match(source, /class="signal-wave"/)
  assert.match(source, /class="wave-svg"/)
  assert.match(source, /class="surface-line is-glow"/)
  assert.match(source, /class="surface-line is-core"/)
  assert.match(source, /:d="surfaceWavePath"/)
  assert.match(source, /const surfaceLifts = ref/)
  assert.match(source, /function buildSurfaceWavePath/)
  assert.match(source, /class="wave-spark"/)
  assert.match(source, /@pointermove\.stop="moveSurfaceWave"/)
  assert.match(source, /@pointerleave\.stop="resetSignalWave"/)
  assert.match(source, /@pointerdown\.stop\.prevent="pulseSignalWave"/)
  assert.match(source, /--surface-hot-color:\s*#6dffea;/)
  assert.match(source, /--surface-cool-color:\s*#8fb6ff;/)
  assert.match(source, /--surface-cursor-x/)
  assert.match(source, /surfaceLifts\.value\s*=\s*SURFACE_POINTS\.map/)
  assert.match(source, /Math\.exp\(-distance \* distance \/ 1\.2\)/)
  assert.match(source, /\.signal-wave\s*\{[^}]*bottom:\s*18px;[^}]*height:\s*76px;[^}]*background:\s*transparent;[^}]*pointer-events:\s*auto;/)
  assert.doesNotMatch(source, /\.signal-wave\s*\{(?:(?!\}).)*border:/s)
  assert.doesNotMatch(source, /\.signal-wave\s*\{(?:(?!\}).)*box-shadow:/s)
  assert.match(source, /\.surface-line\.is-glow\s*\{[^}]*stroke-width:\s*4px;[^}]*filter:\s*drop-shadow/)
  assert.match(source, /\.surface-line\.is-core\s*\{[^}]*stroke-width:\s*1\.8px;[^}]*animation:\s*surfaceBreathe/)
  assert.match(source, /\.continue-panel\.is-wave-pulsing\s+\.wave-svg\s*\{[^}]*animation:\s*wavePulse/)
  assert.match(source, /@keyframes surfaceBreathe/)
  assert.match(source, /@keyframes wavePulse/)
  assert.match(source, /@media \(prefers-reduced-motion: reduce\)[\s\S]*\.surface-line,\s*\.wave-spark,\s*\.wave-svg[\s\S]*?\{[^}]*animation:\s*none;/)
