...
test('article detail hero follows the open prototype briefing layout', () => {
  ...
  assert.match(source, /\.detail-head\s*\{[^}]*align-items:\s*start;/)
  assert.match(source, /\.detail-head\s*\{[^}]*padding:\s*clamp\(8px,\s*2vw,\s*18px\) 0 0;/)
  assert.match(source, /\.detail-head\s*\{[^}]*border:\s*0;/)
  ...
  assert.match(source, /\.detail-actions\s*\{[^}]*padding-bottom:\s*22px;/)
  assert.match(source, /\.detail-actions\s*\{[^}]*border-bottom:\s*1px solid color-mix\(in srgb,\s*var\(--border-color\)\s*66%,\s*transparent\);/)
...
test('article detail summary reads as inline metadata instead of a card', () => {
  ...
  assert.match(source, /\.detail-meta\s*\{[^}]*padding:\s*2px 0 0 18px;/)
  assert.match(source, /\.detail-meta\s*\{[^}]*border-left:\s*1px solid color-mix\(in srgb,\s*var\(--border-color\)\s*72%,\s*transparent\);/)
  assert.match(source, /\.detail-meta\s*\{[^}]*background:\s*transparent;/)
  ...
  assert.match(source, /\.meta-value\s*\{[^}]*font-size:\s*14px;/)
  assert.match(source, /\.meta-value\s*\{[^}]*font-weight:\s*720;/)
